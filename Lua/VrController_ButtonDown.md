...
while not window:Closed() and not window:KeyDown(KEY_ESCAPE) do
    ...
    for n = 1, 2 do
        ...
        if hmd.controllers[n]:ButtonDown(VRBUTTON_PRIMARY) then 
            models[n]:SetColor(1, 0, 0) 
        end
        if hmd.controllers[n]:ButtonDown(VRBUTTON_SECONDARY) then 
            models[n]:SetColor(0, 1, 0) 
        end
        if hmd.controllers[n]:ButtonDown(VRBUTTON_GRIP) then 
            models[n]:SetColor(0, 0, 1) 
        end
        if hmd.controllers[n]:ButtonDown(VRBUTTON_THUMBSTICK) then 
