...
int main(int argc, const char* argv[])
{
    ...
    while (window->Closed() == false and window->KeyDown(KEY_ESCAPE) == false)
    {
        ...
        for (int n = 0; n < 2; ++n)
        {
            ...
            if (hmd->controllers[n]->ButtonHit(VRBUTTON_PRIMARY)) models[n]->SetColor(1, 0, 0);
            if (hmd->controllers[n]->ButtonHit(VRBUTTON_SECONDARY)) models[n]->SetColor(0, 1, 0);
            if (hmd->controllers[n]->ButtonHit(VRBUTTON_GRIP)) models[n]->SetColor(0, 0, 1);
            if (hmd->controllers[n]->ButtonHit(VRBUTTON_THUMBSTICK)) models[n]->SetColor(1, 0.5, 0);
        }
